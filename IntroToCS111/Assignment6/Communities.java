public class Communities {
    ...
    public static double getPM25Level (double[][] data, int commnunity) {
	    ...
	    double airLevel = data[commnunity][HLT_PM2_5];
    ...
    public static double highestPopulationWithHealthIssue (double[][] data, char healthIssue) {
        ...
        if (healthIssue == 'A')
        {
            ...
            {
                if (data[j][HLT_ASTHMA] > max)
                {
                    max = data[j][HLT_ASTHMA];
                }
            }  
        }
        if (healthIssue == 'D')
        {
            for (int j = 0; j < data.length; j++)
            {
                if (data[j][HLT_DIABETES] > max)
                {
                    max = data[j][HLT_DIABETES];
                }
            }
        }
        if (healthIssue == 'H')
        {
            for (int j = 0; j < data.length; j++)
            {
                if (data[j][HLT_HEART_DISEASE] > max)
                {
                    max = data[j][HLT_HEART_DISEASE];
                }
            }
        }
    ...
    public static int numberOfCommunities (double[][] data, int columnIndex, double percentageThreshold, boolean isDisadvantage) {
        ...
        {
            if ((data[k][columnIndex]*100 >= percentageThreshold) && isDisadvantage)
            {
                if (data[k][ECON_DISADVANTAGED] == 1.0)
                count++;
            }
            else if ((data[k][columnIndex]*100 >= percentageThreshold) && !isDisadvantage)
            {
                if (data[k][ECON_DISADVANTAGED] == 0.0)
                    count++;
            }
        }
    ...
    public static double largestRacialDemographic(double[][] data, int community) {
        ...
        double demographic = 0;
        for (int p = 0; p < 8; p++)
        {
            if (data[community][p] > demographic)
            {
                demographic = data[community][p];
            }
        }

        return demographic*100;
    }
    ...
    public static int mostPopulatedDisadvantagedWithLowIncome(double[][] data) {
        int incomeDisadvantaged = 0;
        double largestPopulation = 0;
        int l = 0; 
        
        while (l < data.length)
        {
            if ((data[l][ECON_LOW_INCOME] == 1.0) && (data[l][ECON_DISADVANTAGED] == 1.0))
            {
                if (data[l][TOTAL_POPULATION] > largestPopulation)
                {
                    largestPopulation = data[l][TOTAL_POPULATION];
                    incomeDisadvantaged = l;
                }
            }
            l++;
        }

        return incomeDisadvantaged;
    }

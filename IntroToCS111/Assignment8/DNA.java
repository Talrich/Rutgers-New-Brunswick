...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        for (int i = 0; i < p; i++)
        { 
            personName = StdIn.readLine();
            firstSequence = StdIn.readLine(); 
            secondSequence = StdIn.readLine(); 
            profile = new Profile(personName, S1_STRs, S2_STRs, firstSequence, secondSequence);
            database[i] = profile;
        }
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        for (int i = 0; i < s; i++)
        {
            STRsOfInterest[i] = new String(StdIn.readLine());
        }
    ...
    for (int i = 0; i <= sequence.length() - STR.length(); i++) {
        ...
        while (i + currentRepeats * STR.length() < sequence.length() &&
               sequence.substring(i + currentRepeats * STR.length(),
                                  i + (currentRepeats + 1) * STR.length()).equals(STR)) {
            currentRepeats++;
        }
        ...
        // Reset the loop variable to handle overlapping repeats
        if (currentRepeats > 0) {
            i += (currentRepeats - 1) * STR.length();
        }
    }

    STR STRinSequence = new STR(STR, max);

    return STRinSequence;
}
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        ...
        for (int i = 0; i < allSTRs.length; i++) //for S1_STR[]
        {
            s1 = findSTRInSequence(profile.getSequence1(), allSTRs[i]);
            S1_STR[i] = s1;
            s2 = findSTRInSequence(profile.getSequence2(), allSTRs[i]);
            S2_STR[i] = s2;
        }
        profile.setS1_STRs(S2_STR);
        profile.setS2_STRs(S1_STR);
    ...
    boolean isIdentical = true;  // Assume they are identical until proven otherwise
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        for (int i = 0; i < Math.min(database.length, unknownProfileS1_STRs.length); i++) 
        {
            if (identicalSTRs(unknownProfileS1_STRs, database[i].getS1_STRs()))
            {
                arrList.add(database[i]);
            }
        }

	return arrList; // update the return value
    }
    ...
    // Populate possibleParent1 and possibleParent2 based on STR matches
    for (int i = 0; i < database.length; i++) {
